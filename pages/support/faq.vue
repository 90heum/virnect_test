        ...
        <div class="FAQWrap">
          ...
          <div class="FAQTab">
            <div
              ...
              @click="
                changeTabNextAndPrevMenu(
                  typeList.findIndex((e) => e.id === isCategory),
                  'prev'
                )
              "
            >
...
<script>
import HeadBanner from "~/components/support/faqBanner.vue";
...
export default {
  ...
  methods: {
    ...
    async chooseCategory(e) {
      ...
      try {
        const data = Promise.all([
          this.$axios.$get(
            `admin/support/faq/type?categoryId=${this.isCategory}`
          ),
          this.$axios.$get("admin/support/faq", {
            params: {
    ...
    async chooseType(e, isPaging = false) {
      ...
      try {
        const data = this.$axios.$get("admin/support/faq", {
          params: {
            category: this.isCategory,
            type: e === 0 ? null : e,
            device: null,
  ...
  async asyncData({ $axios, route }) {
    ...
    try {
      const data = Promise.all([
        $axios.$get("admin/support/faq/category"),
        $axios.$get(`admin/support/faq/type?categoryId=${routeCategory}`),
        $axios.$get("admin/support/faq", {
          params: {
...
@media screen and (max-width: 768px) {
  ...
  .LearningCenterMbTab-prev img,
  .LearningCenterMbTab-next img {
    max-width: 42px;
    width: 100%;
  }
